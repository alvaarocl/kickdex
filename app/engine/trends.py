...
def detect_team_trends(df: pd.DataFrame, team: str, venue: str = "all", limit: int = 20) -> list[dict]:
    ...
    matches = _team_match_log(df, team, venue=venue, limit=limit)
    if len(matches) < 5:
        return []
...
def build_trends_payload(df: pd.DataFrame, teams: list[str], season_start=None) -> dict:
    """Build static trends JSON for all teams."""
    source = df.copy()
    if season_start is not None and "Date" in source.columns:
        source["Date"] = pd.to_datetime(source["Date"], errors="coerce")
    ...
    for team in teams:
        result[str(team)] = {
            "all": detect_team_trends(source, team, "all"),
            "home": detect_team_trends(source, team, "home"),
            "away": detect_team_trends(source, team, "away"),
        }
    return {"teams": result}
